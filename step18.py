# Step 18: 定型ファイル名でなければ処理しないようにする
...
def main():
	...
	for filename in filename_list:
		print('========================================')
		print('以下のファイルを読み込み:', filename)

		# 動画ファイルでなければスキップ
		if not filename.endswith('.mov'):
			print('.movファイルでないためスキップ。')
			continue
		# print(filename)
		...
		# ファイル名から区切られたデータ数が足りなければスキップ
		if len(data) < 3:
			print('ファイル名から得られるデータ数が不足しているためスキップ。')
			continue
		...
		# 譜面分類データが辞書のキーになければスキップ
		if not data_difficulty in DICT_DIFFICULTIES.keys():
			print('譜面分類データが適切でないためスキップ。')
			continue
		...
		# 動画ファイル名を変更する
		## 元の動画ファイルパスと名称変更後の動画ファイルパス
		target_file_path = './videos/'+filename
		rename_file_path = './videos/'+filename_renamed
		## 動画ファイル名を変更する
		os.rename(target_file_path, rename_file_path)
		print('名称変更完了:', rename_file_path)
